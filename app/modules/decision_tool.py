...
def _decide_if_order_is_good(tool_input: DecisionInput) -> str:
    ...
    try:
        sanitizer = SnapshotSanitizerTool()
        sanity = sanitizer.run(tool_input=SanitizerInput(record=tool_input.record))
    ...
    segment = str(cell.get("fldfEIZxM3O4pF3bW", "")).strip().upper()
    woj_raw = str(cell.get("fldCbMMnj7vuHlmsu", "")).strip()

    # 2. Mapowanie województwa
    try:
        mapper = WojewodztwoMapperTool()
        mapped = mapper.run(tool_input=MapperInput(wojewodztwo_id=woj_raw))
        if mapped.startswith("❌"):
            logger.warning("Mapowanie nieudane: %s", mapped)
            return f"❌ Nie udało się rozpoznać województwa: {woj_raw}"
    ...
    try:
        excel_path = Path(os.getenv(
            "PREFERENCES_XLSX_PATH", 
            "data/tablica binarna segment + województwo.xlsx"
        ))
        if not excel_path.is_file():
            logger.error("Brak pliku preferencji: %s", excel_path)
            return f"❌ Brak pliku preferencji: {excel_path}"

        logger.info("Otwieram plik preferencji: %s", excel_path)
        ...
        value = df.loc[df["SEGMENT"] == segment, wojewodztwo].values[0]
        result = "TAK" if value == 1 else "NIE"
        logger.info("Decyzja dla segment %s i województwo %s: %s", segment, wojewodztwo, result)
        return result
